...
#messageGens = []
historyPrompt = {}
def enhancePrompt(text, positive=True):
    if(positive):
        text = "the following prompt aims to generate a good quality photograph from StableDiffussion. It is a positive prompt. Please provide an enhanced version as a string starting with ```txt and ending with ```. The text is: " + text
    else:
        text = "the following prompt aims to generate a good quality photograph from StableDiffussion. It is a negative prompt. Please provide an enhanced version as a string starting with ```txt and ending with ```. The text is: " + text
    print(text)
    query = response_query(text)
    print(query)
    prompt = extract_prompt(query)
    if prompt is None:
#        messageGens.clear()
        return text
    # else:
    #     messageGens.append({'role': 'user', 'content': query})
    #     messageGens.append({'role': 'assistant', 'content': prompt})
    print(prompt)
    return prompt
    
def response_query(text):
    ollama_output = ollama.chat(model='deepseek-r1:7b', messages= [
        { 'role': 'user',
          'content': text,
         }, ])
#    messageGens.append({'role': 'user', 'content': text})
#    messageGens.append({'role': 'assistant', 'content': ollama_output.message.content})
    return ollama_output.message.content
...
#         print(json_str)
#         # Load and print the JSON
#         try:
#             data = json.loads(json_str)
#             print("Extracted JSON:")
#             print(json.dumps(data, indent=2))
#             if 'prompt' not in data:
#                 print("Prompt not found in JSON.")
#                 return None
#             return data['prompt']
...
def text2img(prompt,configuration={}):
    ...
    images=[]
    namehash=hashlib.md5(prompt.encode()).hexdigest()
    for i,image in enumerate(imagesAll):
        image.save(namehash + '.' +str(timestamp) +"."+str(i)+"."+ "image.png")
        images.append(namehash + '.' +str(timestamp)+"."+str(i)+"."+"image.png")
        #Write the prompt to a txt file
        with open(namehash + '.' +str(timestamp)+"."+str(i)+"."+"prompt.txt", 'w') as f:
            f.write(prompt)
            f.write("\n")
...
#        creator.create("FitnessMin", base.Fitness, weights=(-1.0,-1.0,-1.0))
    ...
    def evalFitness(self,individual):
        ...
        allimages,namehash=text2img(self.prompt,configuration)
        ...
        if(avgPrecision==0):
            ...
            with open(namehash + ".fitness", 'w') as f:
                f.write(str(avgPrecision/totalCount))
